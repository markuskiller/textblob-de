...
class Chunk(object):
    ...
    def anchor_id(self):
        ...
        id = ""
        f = lambda ch: filter(
            lambda k: self.sentence._anchors[k] == ch,
            self.sentence._anchors)
...
class Sentence(object):
    ...
    def __init__(self, string="", token=[
                 WORD, POS, CHUNK, PNP, REL, ANCHOR, LEMMA], language="en"):
    ...
    def append(self, word, lemma=None, type=None, chunk=None, role=None,
               relation=None, pnp=None, anchor=None, iob=None, custom={}):
    ...
    def parse_token(
        ...
        return p[WORD], p[LEMMA], p[POS], p[CHUNK], p[
            ROLE], p[REL], p[PNP], p[ANCHOR], p[IOB], custom
    ...
    def _do_anchor(self, anchor):
        ...
        if anchor:
            for x in anchor.split("-"):
                ...
                if A in self._anchors and P in self._anchors and not self._anchors[
                        P].anchor:
                    pnp = self._anchors[P]
    ...
    def slice(self, start, stop):
        ...
        for i, word in enumerate(self.words[start:stop]):
            ...
            p5 = word.chunk is not None and unzip(
                0,
                word.chunk.relations) or None  # REL
            p6 = word.chunk is not None and unzip(
                1,
                word.chunk.relations) or None  # ROLE
            # ANCHOR
            ...
            if word.pnp is not None and (
                    word.pnp.start < start or word.chunk.stop > stop):
                p4, p7 = None, None
            s.append(
                word=p0,
                lemma=p1,
    ...
    def __repr__(self):
        return "Sentence(%s)" % repr(
            " ".join(["/".join(word.tags) for word in self.words]).encode("utf-8"))
...
class Text(list):
    ...
    def __init__(self, string, token=[
                 WORD, POS, CHUNK, PNP, REL, ANCHOR, LEMMA], language="en", encoding="utf-8"):
    ...
    for word in sentence.words:
        ...
        if chunk and chunk.start == word.index:
            if chunk.relations:
                ...
                r1 = not len(
                    unique(r1)) == 1 and "|".join(r1) or (
                    r1 +
                    [None])[0]
                r2 = not len(
                    unique(r2)) == 1 and "|".join(r2) or (
                    r2 +
                    [None])[0]
            xml.append(indent + '<%s%s%s%s%s%s>' % (
                XML_CHUNK,
                chunk.type and ' %s="%s"' % (XML_TYPE, chunk.type) or "",
                chunk.relations and chunk.role is not None and ' %s="%s"' % (
                    XML_RELATION, r2) or "",
        ...
        xml.append(indent + '<%s%s%s%s>%s</%s>' % (
            ...
            (" " +
             " ".join(['%s="%s"' %
                       (k, v) for k, v in word.custom_tags.items() if v is not None])).rstrip(),
...
def parse_string(xml):
    ...
    for sentence in dom(XML_SENTENCE):
        ...
        format = sentence.get(
            XML_TOKEN, [
                WORD, POS, CHUNK, PNP, REL, ANCHOR, LEMMA])
        format = not isinstance(
            format,
            basestring) and format or format.replace(
            " ",
            "").split(",")
...
def _parse_relation(chunk, type="O"):
    ...
    return ";".join(["-".join([x for x in (type, r1, r2) if x])
...
def table(sentence, fill=1, placeholder="-"):
    ...
    return "\n".join(["".join([x[i] for x in columns])
