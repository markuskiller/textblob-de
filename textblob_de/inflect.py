...
from textblob_de._text import Verbs as _Verbs
...
def pluralize(word, pos=NOUN, gender=MALE, role=SUBJECT, custom={}):
    ...
    if pos == NOUN:
        ...
        if w.endswith("ien"):
            return w[:-2] + "um"
        if w.endswith(
                ("au", "ein", "eit", "er", "en", "el", "chen", "mus", u"tät", "tik", "tum", "u")):
            return w
        if w.endswith(
                ("ant", "ei", "enz", "ion", "ist", "or", "schaft", "tur", "ung")):
            return w + "en"
        if w.endswith("in"):
            return w + "nen"
        if w.endswith("nis"):
        ...
        if w.endswith(
                ("all", "and", "ang", "ank", "atz", "auf", "ock", "opf", "uch", "uss")):
            umlaut = w[-3]
...
class Verbs(_Verbs):
    ...
    def find_lemma(self, verb):
        ...
        if not (v.startswith("ge") and v.endswith("t")):  # Probably gerund.
            for prefix in prefixes:
                if v.startswith(prefix) and v[
                        len(prefix):] in self.inflections:
    ...
    def find_lexeme(self, verb):
        ...
        ge = (
            v.startswith(prefix_inseparable) or b.endswith(
                ("r", "t"))) and pt or "ge" + pt
