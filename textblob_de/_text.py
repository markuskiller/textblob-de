...
def _read(path, encoding="utf-8", comment=";;;"):
    ...
    if path:
        ...
        for i, line in enumerate(f):
            line = line.strip(
                codecs.BOM_UTF8) if i == 0 and isinstance(
                line,
                str) else line
            line = line.strip()
...
class Model(object):
    ...
    def classify(self, token, previous=None, next=None, **kwargs):
        ...
        return self._classifier.classify(
            self._v(token, previous, next), **kwargs)

    def apply(self, token, previous=(None, None), next=(None, None)):
        """ Returns a (token, tag)-tuple for the given token,
            in context of the given previous and next (token, tag)-tuples.
        """
        return [token[0], self._classifier.classify(
            self._v(token[0], previous, next))]
                ...
                for e in self[w]:
                    ...
                    if b:
                        for token in tokens[i:i + j + 1]:
                            token[1] = token[1] if token[1].startswith(
                                self.tag) else self.tag
                            token[1] += tag
                        i += j
                        break
            i += 1
...
class Parser(object):
    ...
    def __init__(self, lexicon={}, frequency={}, model=None, morphology=None,
                 context=None, entities=None, default=("NN", "NNP", "CD"), language=None):
...
class TaggedString(unicode):
    ...
    def __new__(self, string, tags=["word"], language=None):
        ...
        if isinstance(string, list):
            ...
            string = "\n".join(
                " ".join(
                    "/".join(token) for token in s) for s in string)
    ...
    while j < len(tokens):
        if tokens[j] in ("...", ".", "!", "?", EOS):
            while j < len(tokens) \
                ...
                if tokens[j] in (
                        "'", "\"") and sentences[-1].count(tokens[j]) % 2 == 0:
...
def _suffix_rules(token, tag="NN"):
    ...
    if token.endswith(("able", "al", "ful", "ible", "ient",
                       "ish", "ive", "less", "tic", "ous")) or "-" in token:
        tag = "JJ"
...
def find_tags(tokens, lexicon={}, model=None, morphology=None, context=None,
              entities=None, default=("NN", "NNP", "CD"), language="en", map=None, **kwargs):
        ...
        if chunk[2].endswith("PP") and chunk[-1] == "O":
            ...
            if i < len(chunked) - 1 and \
                ...
                for ch in chunked[i + 1:]:
                    if not (
                            ch[2].endswith(("NP", "PP")) or ch[1] in ("VBG", "VBN")):
                        break
...
def find_relations(chunked):
    ...
    for i, chunk in enumerate(chunks):
        ...
        if tag(chunk[-1]) == "VP" and i < len(chunks) - \
                1 and tag(chunks[i + 1][-1]) == "NP":
            if chunk[-1][-1] == "O":
                id += 1
            for token in chunk:
                token[-1] = "VP-" + str(id)
    ...
    for i, chunk in enumerate(chunks):
        if 0 < i < len(
                chunks) - 1 and len(chunk) == 1 and chunk[-1][-1] == "O":
            t0, t1, t2 = chunks[i -
                                1][-
                                   1], chunks[i][0], chunks[i +
                                                            1][0]  # previous / current / next
    ...
    for i, chunk in enumerate(chunks):
        ...
        for w in chunk:
            if w[1].startswith("NN"):
                ...
                # Higher score for chunks that appear in more contexts
                # (semantic centrality).
...
def commandline(parse=Parser().parse):
    ...
    p.add_option(
        ...
        dest="tags",
        action="store_true",
        help="parse part-of-speech tags")
    p.add_option(
        "-C",
        "--chunks",
        dest="chunks",
...
class Sentiment(lazydict):
    ...
    def synset(self, id, pos=ADJECTIVE):
        ...
        try:
            ...
            return tuple(
                self._synsets.get(re.sub(r"-0+", "-", id), (0.0, 0.0))[:2])
    ...
    def assessments(self, words=[], negation=True):
        ...
        for w, pos in words:
            ...
            if w in self and pos in self[w]:
                ...
                if w.isalpha() is False and len(
                        w) <= 5 and w not in PUNCTUATION:  # speedup
    ...
    def annotate(self, word, pos=None, polarity=0.0,
                 subjectivity=0.0, intensity=1.0, label=None):
