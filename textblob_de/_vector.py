...
try:
    from textblob_de._text import singularize, predicative, conjugate, tokenize
except:
    try:
        import sys
...
def words(string, filter=lambda w: w.strip(
        "'").isalnum(), punctuation=PUNCTUATION, **kwargs):
    ...
    string = re.sub(
        r"([a-z|A-Z])'(m|s|ve|re|ll|d)",
        u"\\1 <QUOTE/>\\2",
        string)
    string = re.sub(
        r"(c|d|gl|j|l|m|n|s|t|un)'([a-z|A-Z])",
        u"\\1<QUOTE/> \\2",
        string)
    words = (w.strip(punctuation).replace(u"<QUOTE/>", "'", 1)
...
def count(words=[], top=None, threshold=0, stemmer=None,
          exclude=[], stopwords=False, language=None, **kwargs):
    ...
    for w in words:
        ...
        if isinstance(w, basestring):
            w1 = w.lower()
            w2 = w.lower()
        if (stopwords or not w1 in _stopwords.get(language or "en", ())
            ) and not w1 in exclude:
    ...
    if top is not None:
        count = count.__class__(
            heapq.nsmallest(top, count.iteritems(), key=lambda kv: (-kv[1], kv[0])))
    return count
...
class Document(object):
    ...
    def concepts(self):
        ...
        """
        return self.model and self.model.lsa and self.model.lsa.concepts.get(
            self.id) or None
...
def distance(v1, v2, method=COSINE):
    ...
    if method == EUCLIDEAN:
        return sum(
            (v1.get(w, 0) - v2.get(w, 0)) ** 2 for w in set(chain(v1, v2)))
    if method == MANHATTAN:
        return sum(abs(v1.get(w, 0) - v2.get(w, 0))
                   for w in set(chain(v1, v2)))
    if method == CHEBYSHEV:
        return max(abs(v1.get(w, 0) - v2.get(w, 0))
                   for w in set(chain(v1, v2)))
    if method == HAMMING:
        d = sum(
            not (
                w in v1 and w in v2 and v1[w] == v2[w]) for w in set(
                chain(
                    v1,
                    v2)))
...
class Model(object):
    ...
    def density(self):
        ...
        """
        return float(sum(len(d.vector)
                         for d in self.documents)) / len(self.vector) ** 2
...
#   def cluster(self, method=HIERARCHICAL, k=1, iterations=1000)
    def cluster(self, method=KMEANS, **kwargs):
        ...
        if not getattr(self, "lsa", None):
            ...
            vectors, features = [
                d.vector for d in documents], self.vector.keys()
        else:
            # Using LSA concept space:
            vectors, features = [self.lsa[d.id]
                                 for d in documents], range(len(self.lsa))
    ...
    def feature_selection(self, top=100, method=INFOGAIN, threshold=0.0):
        ...
        subset = [
            w for x,
            w in subset[
                :top if top is not None else len(subset)]]
        return subset
...
class LSA(object):
    ...
    def concepts(self):
        ...
        # 0.0.
        return [dict((self._terms[i], round(w, 15))
                     for i, w in enumerate(concept)) for concept in self.vt]
...
def k_means(vectors, k=None, iterations=10,
            distance=COSINE, seed=RANDOM, **kwargs):
...
class Classifier(object):
    ...
    def baseline(self):
        ...
        return ([(0, None)] + sorted([(v, k)
                                      for k, v in self._classes.iteritems()]))[-1][1]
    ...
    def skewness(self):
        ...
        a = list(
            chain(*([i] * v for i, (k, v) in enumerate(self._classes.iteritems()))))
    ...
    def k_fold_cross_validation(cls, corpus=[], k=10, **kwargs):
        ...
        return K_fold_cross_validation(
            cls, documents=corpus, folds=k, **kwargs)
    ...
    def test(cls, corpus=[], d=0.65, folds=1, **kwargs):
        ...
        if folds > 1:
            return K_fold_cross_validation(
                cls, documents=corpus, folds=folds, **kwargs)
    ...
    def confusion_matrix(self, documents=[]):
        ...
        documents = [
            isinstance(
                ...
                Document) and (
                d,
                d.type) or d for d in documents]
...
class ConfusionMatrix(defaultdict):
    ...
    def auc(self, k=10):
        ...
        return sum(0.5 * (x1 - x0) * (y1 + y0)
                   for (x0, y0), (x1, y1) in sorted(zip(roc, roc[1:])))
...
class NB(Classifier):
    ...
    def __init__(
            self, train=[], baseline=MAJORITY, method=MULTINOMIAL, alpha=0.0001):
    ...
    def classify(self, document, discrete=True):
            ...
            if m == MULTINOMIAL:
                if not type in self._cache:  # 10x faster
                    self._cache[type] = float(
                        sum(self._likelihood[type].itervalues()))
                d = self._cache[type]
            if m == BINOMIAL \
                    or m == BERNOUILLI:
        ...
        try:
            ...
            p = sorted((self._classes[type], type)
                       for type, g in p.iteritems() if g == m > 0)
...
class KNN(Classifier):
    ...
    def classify(self, document, discrete=True):
        ...
        try:
            ...
            m = max(p.itervalues())
            p = sorted((self._classes[type], type)
                       for type, w in p.iteritems() if w == m > 0)
            p = [type for frequency, type in p if frequency == p[0][0]]
            return choice(p)
        except:
            return self.baseline
...
class SLP(Classifier):
    ...
    def features(self):
        return list(
            set(chain(*(f.iterkeys() for f in self._weight.itervalues()))))
...
class BPNN(Classifier):
    ...
    def __init__(
            self, train=[], baseline=MAJORITY, layers=2, iterations=1000, **kwargs):
    ...
    def _weight_initialization(
            self, i=1, o=1, hidden=1, method=RANDOM, a=0.0, b=1.0):
...
class SVM(Classifier):
    ...
    def _classify(self, document, probability=False):
        ...
        v = self._vector(document)[1]
        v = dict(
            map(lambda k: (H1.get(k[1], k[0] + n + 1), v[k[1]]), enumerate(v)))
        ...
        if self._type == CLASSIFICATION and probability is True:
            return defaultdict(float, ((H3[i], w)
                                       for i, w in enumerate(p[2][0])))
...
class GeneticAlgorithm(object):
    ...
    def avg(self):
        ...
        return float(sum(map(self.fitness, self.population))) / \
            len(self.population)
